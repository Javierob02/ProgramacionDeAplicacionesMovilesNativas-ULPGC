...
class BusScheduleViewModel(private val busScheduleDao: BusScheduleDao): ViewModel() {
    ...
    fun getAllSchedule(): Flow<List<BusSchedule>> = busScheduleDao.getAll()    //Obtenemos todas las paradas con nuestra función getAll()
    // Get bus schedule based on the stop name from Room DB
    fun getStopSchedule(stopName: String): Flow<List<BusSchedule>> =
        busScheduleDao.getByName(stopName)
